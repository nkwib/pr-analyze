<script>
  ...
  const links = [
    ...
    { href: '/output-schema', label: 'Output' },
    { href: '/api', label: 'API' },
...
<style>
  ...
  @media (max-width: 720px) {
    .inner { grid-template-columns: auto auto; gap: var(--sp-3); }
    .nav {
      grid-column: 1 / -1; justify-self: stretch; justify-content: flex-start;
      overflow-x: auto; padding-bottom: 4px; scrollbar-width: none; order: 3;
    }
    .nav::-webkit-scrollbar { display: none; }
    .header { height: auto; }
    .inner { padding-top: var(--sp-3); padding-bottom: var(--sp-3); }
  }
</style>
